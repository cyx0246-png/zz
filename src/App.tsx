...
export default function App() {
  ...
  const handleAnswer = (option: Option) => {
    setScoreHistory([...scoreHistory, dimensionScores]);
  ...
  const resultData = useMemo(() => {
    if (screen !== 'result') return { character: characters[0], matchPercentage: 0 };
    ...
    // 将距离转换为百分比 (最大合理偏差设定为约150)
    let percentage = Math.round(100 - (minDistance / 1.5));
    if (percentage > 99) percentage = 99; // 留有一丝神秘感
    if (percentage < 60) percentage = 60 + Math.floor(Math.random() * 10); // 设定保底分数，体验更好

    return { character, matchPercentage: percentage };
  }, [dimensionScores, screen]);

  const resultCharacter = resultData.character;
  const matchPercentage = resultData.matchPercentage;
                ...
                <div className="flex-grow space-y-2 md:space-y-8 overflow-y-auto max-h-[70vh] md:max-h-none pr-1">
                  ...
                  {currentQuestionIdx > 0 && (
                    <div className="flex justify-start pt-2">
                      <button
                        onClick={handleBack}
                        className="flex items-center text-[10px] md:text-sm text-text-muted hover:text-official-red transition-colors group"
                      >
                        <svg className="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2 transform group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div className="space-y-6 py-2">
                    <div className="grid grid-cols-2 gap-4 border-b border-dashed border-[#ccc] pb-4 font-sans text-xs">
                      ...
                      <div>
                        ...
                        <span className="text-official-red font-black text-lg tracking-widest">{matchPercentage}%</span>
                      </div>
                    </div>
